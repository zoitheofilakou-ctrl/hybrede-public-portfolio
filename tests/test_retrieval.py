...
class RetrievalTests(unittest.TestCase):
    def test_manifest_abstract_selection_overrides_existing_fulltext(self):
        paper = {
            "paperId": "target",
            "title": "Target",
        ...
        with tempfile.TemporaryDirectory() as directory:
            Path(directory, "target.txt").write_text("historical invitation text", encoding="utf-8")
            result = retrieval.get_text_for_paper(
                paper,
                fulltext_dir=directory,
                source_entry={"paperId": "target", "source_type": "abstract"},
            )
        self.assertEqual("Structured abstract text", result[2])
        self.assertEqual("abstract", result[4])
    ...
    def test_hybrid_query_result_excludes_weak_lexical_only_candidate(self):
        ...
        def score_fixture(_analysis, papers):
            for paper in papers:
                paper["cross_encoder_score"] = 0.80 if paper["chunk_id"] == "vec:0000" else 0.40
            return "cross-encoder fixture scores"

        with patch("Retrieval.retrieval.apply_cross_encoder", side_effect=score_fixture):
    ...
    def test_hybrid_query_result_can_rescue_strong_lexical_only_candidate(self):
        ...
        def score_fixture(_analysis, papers):
            for paper in papers:
                paper["cross_encoder_score"] = 0.80
            return "cross-encoder fixture scores"
    ...
    def test_build_result_row_embeds_score_breakdown_in_metadata(self):
        ...
        self.assertEqual(0.567, row["final_scores"][0])
        self.assertEqual(0.567, metadata["final_score"])
