"""Immutable-source / disposable-runtime index controls.

Scientific-integrity utility used by the HyBreDe evaluation harness.

The purpose is to guarantee that an evaluation run can never mutate the
canonical (frozen) index. A run operates on a verified byte-identical
disposable copy, and the canonical source can be re-checked afterwards to
prove it did not change.

Author: Zoi Theofilakou. Reproduced here unmodified apart from this docstring.
"""
...
def assert_source_unchanged(source: Path, expected_tree_sha256: str) -> None:
    ...
    if observed != expected_tree_sha256:
        raise RuntimeError(
            f"immutable source changed: expected {expected_tree_sha256}, "
            f"observed {observed}"
        )
