...
DEFAULT_SOURCE_MANIFEST_FILE = CORPUS_SOURCE_MANIFEST_PATH
...
def get_embedding_model():
    ...
    try:
        ...
        raise RuntimeError(
            f"Required local embedding model unavailable: {EMBED_MODEL_NAME}: {exc}"
        ) from exc
...
def get_cross_encoder_model():
    ...
    try:
        ...
        raise RuntimeError(
            f"Required local cross-encoder unavailable: {CROSS_ENCODER_MODEL_NAME}: {exc}"
        ) from exc
...
def resolve_manifest_path(relative_or_absolute: str) -> str:
    """Resolve a source-manifest text path.

    PUBLIC RELEASE SANITIZATION. Resolution order: an absolute path is used as
    given; otherwise the repository root is tried first (the original,
    frozen-manifest behaviour) and the configured data root second, so a corpus
    supplied through HYBREDE_DATA_DIR works without editing the manifest.
    """
    if os.path.isabs(relative_or_absolute):
        return relative_or_absolute
    repo_relative = os.path.join(BASE_DIR, relative_or_absolute)
    if os.path.isfile(repo_relative):
        return repo_relative
    return os.path.join(DATA_DIR, relative_or_absolute)
...
def load_source_manifest(
    ...
    """Load and validate the explicit paperId source-selection manifest."""
    if not os.path.exists(manifest_file):
        raise RuntimeError(f"Source manifest missing: {manifest_file}")
    with open(manifest_file, "r", encoding="utf-8") as f:
        payload = json.load(f)
    records = payload.get("records") if isinstance(payload, dict) else payload
    if not isinstance(records, list):
        raise RuntimeError("Source manifest must contain a records list")
    result: Dict[str, dict] = {}
    for record in records:
        pid = (record.get("paperId") or "").strip() if isinstance(record, dict) else ""
        source_type = record.get("source_type") if isinstance(record, dict) else None
        if not pid or pid in result:
            raise RuntimeError("Source manifest contains a missing or duplicate paperId")
        if source_type not in {"fulltext", "abstract", "non_evidence_bearing"}:
            raise RuntimeError(f"Invalid source declaration for {pid}: {source_type}")
        selected = record.get("selected_text_path") if source_type == "fulltext" else ""
        if source_type == "fulltext":
            if not selected:
                raise RuntimeError(f"Full-text source path missing for {pid}")
            # PUBLIC RELEASE SANITIZATION: manifest paths were always written
            # relative to the repository root, because the corpus lived inside
            # the repository. A user-supplied corpus (HYBREDE_DATA_DIR) may sit
            # anywhere, so an absolute path is honoured and the data root is
            # tried as a fallback. Repository-relative paths still resolve
            # exactly as before, so a frozen manifest is unaffected.
            selected_path = resolve_manifest_path(selected)
            if not os.path.isfile(selected_path):
                raise RuntimeError(f"Selected full-text file missing for {pid}: {selected}")
        result[pid] = record
    if included_ids is not None and set(result) != set(included_ids):
        raise RuntimeError("Source manifest paperId set conflicts with filtered corpus")
    return result
...
def get_text_for_paper(
    ...
    if source_entry is not None and source_entry.get("source_type") == "abstract":
        full_text = ""
    elif source_entry is not None and source_entry.get("source_type") == "non_evidence_bearing":
        return title, abstract_text, "", year, "non_evidence_bearing"
    elif source_entry is not None and source_entry.get("source_type") == "fulltext":
        selected = source_entry.get("selected_text_path")
        if not selected:
            raise RuntimeError(f"Full-text source path missing for {paper_id}")
        # PUBLIC RELEASE SANITIZATION: see resolve_manifest_path.
        selected_path = resolve_manifest_path(selected)
        if not os.path.isfile(selected_path):
            raise RuntimeError(f"Selected full-text file missing for {paper_id}: {selected}")
...
def aggregate_papers(candidates: List[dict]) -> List[dict]:
    ...
    for paper_id, paper_candidates in grouped.items():
        ...
        # Preserve the already-generated candidate set for deterministic,
        # proposition-aware evidence exposure after paper ranking.  This does
        # not alter candidate scores, paper aggregation, or selected-paper order.
        representative["candidate_chunks"] = [
            {
                "chunk_id": candidate["chunk_id"],
                "paperId": candidate.get("paperId"),
                "text": candidate.get("text", ""),
                "section": candidate.get("section", "body"),
                "chunk_score": candidate.get("chunk_score", 0.0),
            }
            ...
            enrich_metadata_with_scores(
                {
                    ...
                    "candidate_chunks": paper.get("candidate_chunks", []),
...
def cmd_index(metadata_file: str, filtered_file: str, source_manifest_file: str = DEFAULT_SOURCE_MANIFEST_FILE):
    require_retrieval_dependencies()
    ...
    source_manifest = load_source_manifest(source_manifest_file, included_ids=allowed_set)
    ...
    for pid in allowed_ids:
        ...
        source_entry = source_manifest[pid]
        if source_entry.get("source_type") == "non_evidence_bearing":
            continue
        full_text = "" if source_entry.get("source_type") == "abstract" else None
        text_source = "fulltext" if full_text else "abstract"
        if source_entry.get("source_type") == "fulltext":
            fulltext_papers += 1
        else:
...
def main():
    ...
    p_index.add_argument("--source-manifest", default=DEFAULT_SOURCE_MANIFEST_FILE, help="Explicit source-selection manifest")
    ...
    if args.cmd == "index":
        cmd_index(args.metadata, args.filtered, args.source_manifest)
    elif args.cmd == "query":
